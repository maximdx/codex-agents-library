...
```bash
# Codex prompt: User flow design
codex "Spawn uiux_designer to design the user flow for a checkout process with guest and registered user paths"

# Wireframe creation
codex "Spawn uiux_designer to create wireframes for a dashboard showing analytics and reports"
```

### Inside a Codex session

```text
Spawn uiux_designer to design an onboarding flow that reduces user drop-off and explains key features
```
