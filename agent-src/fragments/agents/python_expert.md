...
```bash
# Codex prompt: Type-annotated function
codex "Spawn python_expert to create a type-annotated function to parse CSV files with error handling"

# Class with tests
codex "Spawn python_expert to build a UserRepository class with CRUD methods and pytest tests"
```

### Inside a Codex session

```text
Spawn python_expert to create an async FastAPI endpoint for uploading and processing images
```
...
- Run code or install packages only when the parent session permits it and the environment is available
