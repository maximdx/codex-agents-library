...
```bash
# Codex prompt: Create and schedule a post
codex "Spawn buffer_api to write and schedule a LinkedIn post for tomorrow at 9am UTC announcing our Q2 results"

# Improve existing post copy
codex "Spawn buffer_api to improve this post for LinkedIn and X, keeping it concise and professional: [paste draft]"

# Pull queued and scheduled posts
codex "Spawn buffer_api to pull all queued and scheduled posts for my LinkedIn channels this week"

# Edit a queued post
codex "Spawn buffer_api to update this queued post to sound more helpful and less salesy, then keep the same scheduled time: [post ID]"

# Analyze performance and suggest improvements
codex "Spawn buffer_api to analyze sent posts from the last 30 days and suggest what to post more of next week"

# Build a posting automation script
codex "Spawn buffer_api to write a Python script that reads posts from a CSV, improves platform-specific copy, and schedules them via Buffer API"
```

### Inside a Codex session

```text
Spawn buffer_api to improve this launch announcement for LinkedIn and X, then schedule it to the right Buffer channels for next Monday at 10am UTC
```

**Example scenarios:**
- Post and schedule: `Spawn buffer_api to create a scheduled Instagram post with text and image URL for Friday 3pm UTC`
- Improve content: `Spawn buffer_api to rewrite these 5 draft posts for LinkedIn, X, and Instagram`
- Pull posts: `Spawn buffer_api to list queued, scheduled, and sent posts for this campaign`
- Edit posts: `Spawn buffer_api to update the scheduled launch post to include the new pricing link`
- Analyze performance: `Spawn buffer_api to fetch this month's post metrics and suggest better hooks for next week`
- Manage ideas: `Spawn buffer_api to create 5 content ideas for a product launch campaign`
