...
The file links below point to the checked-in standalone TOMLs that Codex loads. Contributors should make changes through `agent-src/` and run `python3 scripts/generate-agents.py`; direct edits to `.codex/agents/` are overwritten. Every preset uses the shared agent contract. The Python, JavaScript/TypeScript, Go, Java, Rust, and SQL presets additionally compose a Software Engineer base with a focused overlay while preserving their public names and routing metadata.
...
- Defaults: `gpt-5.6-terra`, `medium` reasoning, `workspace-write` sandbox
...
- Defaults: `gpt-5.6-terra`, `medium` reasoning, `read-only` sandbox
...
- Defaults: `gpt-5.6-terra`, `high` reasoning, `workspace-write` sandbox
...
- Defaults: `gpt-5.6-luna`, `low` reasoning, `read-only` sandbox
...
- Defaults: `gpt-5.6-sol`, `high` reasoning, `read-only` sandbox
...
- Defaults: `gpt-5.6-luna`, `medium` reasoning, `workspace-write` sandbox
...
- Defaults: `gpt-5.6-terra`, `high` reasoning, `read-only` sandbox
